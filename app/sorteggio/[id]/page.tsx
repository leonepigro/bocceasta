...
export default async function SorteggioPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await service
    .from('draft_sessions')
    .select('id, season, result, locked_at, applied_at')
    .eq('id', id)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--background)' }}>
      <header className="text-white px-4 py-4 text-center shadow-md" style={{ background: 'var(--boccea-red)' }}>
        <h1 className="text-xl font-black">🎲 Sorteggio Rose {data.season}</h1>
        <p className="text-xs opacity-75 mt-1">
          Bloccato il {new Date(data.locked_at).toLocaleString('it-IT', {
